...
function PlatformHub() {
  ...
  // D-4.2: URL-based navigation — ?tab=home|war|chat|store|profile
  const searchParams = useSearchParams();
  const urlTab = (searchParams.get("tab") as Tab) ?? "home";
  const [tab, setTab] = useState<Tab>(urlTab);
  const [onlineCount, setOnlineCount] = useState<number | null>(null);

  // sync tab → URL without full reload
...
// D-4.2: Wrap في Suspense عشان useSearchParams
export default function PlatformHubWrapper() {
  return (
    <Suspense fallback={
      <div style={{ minHeight:"100dvh", background:"#0c0c0e", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ color:"rgba(255,255,255,0.2)", fontSize:14 }}>جاري التحميل...</div>
      </div>
    }>
      <PlatformHub />
    </Suspense>
