...
import { getTier, getTierProgress } from "@/lib/platform/elo";
...
export default function DominoLobby() {
  ...
  useEffect(()=>{
    ...
    // جلب عدد اللاعبين أونلاين
    fetch("/api/online-count").then(r=>r.json()).then(d=>setOnlineCount(d.count??null)).catch(()=>{});
  ...
  const tier  = getTier(profile?.elo ?? 1000);
  const tierPct = getTierProgress(profile?.elo ?? 1000);
      ...
      <div className="min-h-dvh relative overflow-hidden text-white" style={{background:"#07090f"}} dir="rtl">
        ...
        <header className="relative z-20 flex items-center justify-between gap-2 px-4 py-3 md:px-8 md:pt-6"
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {/* D-2.5: Tier Badge */}
            <div className="hidden sm:flex flex-col items-center px-3 py-1.5 rounded-xl"
              style={{background:"rgba(255,255,255,.04)",border:`1px solid ${tier.color}30`}}>
              <span className="text-[9px] font-black uppercase tracking-widest" style={{color:tier.color}}>{tier.icon} {tier.name}</span>
              <div className="mt-1 w-14 h-1 rounded-full overflow-hidden" style={{background:"rgba(255,255,255,.07)"}}>
                <div className="h-full rounded-full" style={{width:`${tierPct}%`,background:tier.color,transition:"width .8s ease"}}/>
              </div>
            </div>
          ...
          <div className="w-full lg:w-72 flex flex-col gap-4">
            ...
            {onlineCount !== null && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.3}}
                ...
                style={{background:"rgba(0,255,136,.04)",border:"1px solid rgba(0,255,136,.12)"}}>
                ...
                <span className="font-black text-xs" style={{color:"#00ff88"}}>{onlineCount.toLocaleString()} لاعب أونلاين الآن</span>
              </motion.div>
            )}
            ...
            <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:.54}}
              ...
              <Link href="/games/domino/tournaments"
