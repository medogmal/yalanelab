...
export default function ChessGameOnline2D({ initialAiDifficulty = 10 }: { initialAiDifficulty?: number }) {
  const { user, equipped, unlockItem, language, culturalMood } = usePlatformStore();
  const cTheme = getTheme(culturalMood);
  const { socket } = useSocket();
  
  // لو جاء initialAiDifficulty من StageSelector → ابدأ مباشرة في AI mode
  const autoStart = initialAiDifficulty !== 10;
  ...
  // AI Settings
  const [aiDifficulty, setAiDifficulty] = useState(initialAiDifficulty); // 0-20
  ...
  // Get Theme — safe null check
  const themeKey = ((equipped?.chess_skin ?? "").replace("skin_", "") || "classic") as keyof typeof THEMES;
  ...
  // --- Effects ---
  // لو جاي من StageSelector → ابدأ تلقائياً بـ AI mode
  useEffect(() => {
    if (autoStart) {
      startGame("ai", TIME_CONTROLS[1], initialAiDifficulty, "random");
    }
  ...
  if (phase === "game") {
    ...
    return (
      <div className={`flex flex-col min-h-screen min-h-dvh ${theme.bg}`} style={
        ...
        <div className="flex-1 flex items-center justify-center p-2 overflow-hidden">
          <div className="relative aspect-square shadow-2xl rounded-sm overflow-hidden border-4 border-black/20"
               style={{ width: "min(96vw, calc(100dvh - 180px), 560px)" }}>
