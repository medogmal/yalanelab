...
export default function ProfessionalCard({
  ...
  const getBackPattern = () => {
    ...
    if (skin === "neon") return (
        <div className="w-full h-full bg-black rounded-lg border border-purple-500 flex items-center justify-center relative overflow-hidden">
    ...
    return (
      <div className={`w-full h-full bg-white rounded-lg flex flex-col relative overflow-hidden select-none ${skin === "neon" ? "bg-zinc-900 text-white border border-purple-500/50" : "bg-white"}`}>
        ...
        <div className="absolute top-2 left-2 flex flex-col items-center leading-none">
          ...
          <SuitIcon suit={suit} className={`w-4 h-4 ${skin === "neon" && isRed ? "text-pink-500" : skin === "neon" ? "text-cyan-400" : colorClass}`} />
        </div>
  ...
  return (
    <div 
      className={`
        ...
        ${bgClass}
        ${borderClass}
        ${disabled ? "opacity-50 cursor-not-allowed grayscale" : "cursor-pointer hover:-translate-y-4 hover:rotate-1"}
      `}
      style={{ 
        width, 
        height,
