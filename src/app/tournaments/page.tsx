    ...
    try {
      ...
      if (data.ok) {
        ...
        setTournaments(prev => prev.map(x =>
          x.id === t.id ? { ...x, currentParticipants: x.currentParticipants + 1 } : x
        ));
      } else {
        alert(data.error === "already_registered" ? "أنت مسجل بالفعل!"
          : data.error === "insufficient_coins" ? "رصيدك غير كافٍ"
              ...
              return (
                <motion.div key={t.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  ...
                  <div style={{ padding: "14px 14px 16px" }}>
                    ...
                    {/* CTA */}
                    <button
                      disabled={t.status !== "upcoming" || registering === t.id || registered.has(t.id)}
                      onClick={() => t.status === "upcoming" && !registered.has(t.id) && handleRegister(t)}
                      style={{
                        width: "100%", padding: "10px", borderRadius: 11, border: "none",
                        cursor: t.status === "upcoming" && !registered.has(t.id) ? "pointer" : "not-allowed",
                        background: registered.has(t.id) ? "#065f46"
                          : t.status === "upcoming" ? (registering === t.id ? "#5b21b6" : "#7c3aed")
                          : "#1e1e25",
                        color: t.status === "upcoming" || registered.has(t.id) ? "#fff" : "#404050",
                        fontWeight: 800, fontSize: 13, fontFamily: "inherit", transition: "all .15s",
                      }}
                    >
                      {registered.has(t.id) ? "✓ مسجّل"
                        : registering === t.id ? "..."
                        : t.status === "upcoming" ? "سجّل الآن"
