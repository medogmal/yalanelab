...
import { getMemory, joinLobby, leaveLobby, pairPlayers } from "@/lib/ludo/server";
...
export async function GET(req: NextRequest) {
  ...
  if (forId) {
    const m = Array.from(mem.matches.values()).find((mt) => mt.a.id === forId || mt.b.id === forId || mt.c.id === forId || mt.d.id === forId);
    if (m) {
      return Response.json({
        match: {
          ...
          c: { id: m.c.id, name: m.c.name },
          d: { id: m.d.id, name: m.d.name },
  ...
  return Response.json({
    ...
    match: match
      ? {
          ...
          c: { id: match.c.id, name: match.c.name },
          d: { id: match.d.id, name: match.d.name },
