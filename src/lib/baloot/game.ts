...
function rankScore(card: Card, mode: Mode, trump: Suit | null, lead: Suit | null) {
  // D-4.6: تم حذف التكرار — شرط trump مرة واحدة فقط
...
export class BalootGame {
  ...
  private outranks(newBid: Bid, cur: Bid | null) {
    ...
    if (cur.mode === "sun") return false;
    // كلاهما hokom — اقبل فقط لو trump مختلف
    if (newBid.mode === "hokom" && cur.mode === "hokom") {
      return newBid.trump !== cur.trump;
    }
    return false;
  }
  private finalizeBidIfReady() {
    // لو كل اللاعبين passوا بدون أي bid — ابدأ بـ hokom عشوائي
    if (!this.currentBid && this.passesInRow >= 4) {
      const suits: Suit[] = ["S", "H", "D", "C"];
      const randomTrump = suits[Math.floor(Math.random() * suits.length)];
      this.currentBid = { mode: "hokom", trump: randomTrump };
      this.bidWinner = this.players[(this.bidderIndex) % this.players.length];
      this.passesInRow = 4; // FIX 4.7: 4 مش 3 عشان الـ state صح
    }
  ...
  legalCards(pid: PlayerId) {
    ...
    // Trump Forcing: لو ما عندوش في السويت، يلعب trump إجباري
    if (this.mode === "hokom" && this.trump) {
