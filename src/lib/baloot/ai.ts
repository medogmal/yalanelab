...
export function getBestBid(game: BalootGame, pid: PlayerId): Bid | "pass" {
  ...
  const bidderIdx = game.players.indexOf(pid);
  const dealerIdx = game.players.indexOf(game.dealer);
  // آخر لاعب قبل ما تبدأ اللعبة (الـ dealer) يبيد دايماً لو محدش بيد
  const isLastBidder = ((dealerIdx + 4) % 4) === bidderIdx;

  if (!current) {
    if (sunPoints >= 18)  return { mode: "sun" };
    if (maxPoints >= 20)  return { mode: "hokom", trump: maxSuit };
    // آخر بيدر يبيد دايماً عشان اللعبة ماتوقفش
    if (isLastBidder)     return { mode: "hokom", trump: maxSuit };
  } else {
    // لو في bid موجود، يبيد بـ sun لو عنده يد قوية
    if (current.mode === "hokom" && sunPoints >= 22) return { mode: "sun" };
  }
  return "pass";
}
