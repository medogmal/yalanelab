...
// نستخدم ملف منفصل في الذاكرة للأصدقاء (مش في users.json عشان يبقى خفيف)
declare global {
  var __FRIENDS_MEM__: {
    requests: Map<string, Set<string>>;  // targetId → Set<fromId>
    friends:  Map<string, Set<string>>;  // userId   → Set<friendId>
  } | undefined;
}

function getFriendsMem() {
  if (!global.__FRIENDS_MEM__) {
    global.__FRIENDS_MEM__ = {
      requests: new Map(),
      friends:  new Map(),
    };
  }
  return global.__FRIENDS_MEM__;
}

/** جلب قائمة الأصدقاء والطلبات الواردة لمستخدم معين */
export function getFriends(userId: string) {
  const mem = getFriendsMem();
  const friends  = Array.from(mem.friends.get(userId)  ?? []);
  const incoming = Array.from(mem.requests.get(userId) ?? []);
  const users    = loadUsers();

  const toInfo = (id: string) => {
    const u = users.find(x => x.id === id);
    return u ? { id: u.id, name: u.name } : { id, name: "لاعب" };
  };

  return {
    friends:  friends.map(toInfo),
    incoming: incoming.map(toInfo),
  };
}
...
export function sendFriendRequest(fromId: string, toId: string) {
  ...
  // لو هما أصدقاء فعلاً
  if (mem.friends.get(fromId)?.has(toId)) return { ok: false, error: "already_friends" };

  // لو الطرف الآخر أرسل طلب من قبل → اقبل تلقائياً
  if (mem.requests.get(fromId)?.has(toId)) {
    return acceptFriendRequest(fromId, toId);
  }

  if (!mem.requests.has(toId)) mem.requests.set(toId, new Set());
  mem.requests.get(toId)!.add(fromId);
  return { ok: true };
}

/** قبول طلب صداقة */
export function acceptFriendRequest(userId: string, fromId: string) {
  const mem = getFriendsMem();
  const incoming = mem.requests.get(userId);
  if (!incoming?.has(fromId)) return { ok: false, error: "no_request" };

  incoming.delete(fromId);
  if (!mem.friends.has(userId)) mem.friends.set(userId, new Set());
  if (!mem.friends.has(fromId)) mem.friends.set(fromId, new Set());
  mem.friends.get(userId)!.add(fromId);
  mem.friends.get(fromId)!.add(userId);
  return { ok: true };
}
...
export function removeFriend(userId: string, targetId: string) {
  const mem = getFriendsMem();
  mem.friends.get(userId)?.delete(targetId);
  mem.friends.get(targetId)?.delete(userId);
  mem.requests.get(userId)?.delete(targetId);
  mem.requests.get(targetId)?.delete(userId);
  return { ok: true };
}
